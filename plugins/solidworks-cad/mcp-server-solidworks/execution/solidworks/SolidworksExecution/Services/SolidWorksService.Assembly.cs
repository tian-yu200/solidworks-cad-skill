...
namespace SolidworksExecution.Services
{
    ...
    public partial class SolidWorksService
    {
        ...
        private static readonly Dictionary<string, string> ToolboxNameHints =
            new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
            {
                { "5780", "hexagon head bolts" },
                { "5781", "hexagon head bolts full thread" },
                { "5782", "hexagon head bolts" },
                { "5783", "hexagon head bolts full thread" },
                { "70.1", "hexagon socket head cap screws" },
                { "70.2", "hexagon socket countersunk head screws" },
                { "70.3", "hexagon socket button head screws" },
                { "6170", "hex nuts" },
                { "6172.1", "hexagon thin nuts" },
                { "6172.2", "hexagon thin nuts" },
                { "6175", "hexagon nuts with flange" },
                { "97.1", "plain washers" },
                { "97.2", "plain washers" },
                { "93", "plain washers" },
                { "95", "plain washers" }
        ...
        private static IEnumerable<string> ToolboxRoots(string explicitRoot, string standard)
        {
            var roots = new[]
            {
                explicitRoot,
                System.Environment.GetEnvironmentVariable("SW_TOOLBOX_FOLDER"),
                @"C:\SOLIDWORKS Data\browser",
                @"C:\SOLIDWORKS Data\Toolbox",
                @"D:\SOLIDWORKS Data\browser",
                @"D:\SOLIDWORKS Data\Toolbox",
                @"C:\Program Files\SOLIDWORKS Corp\SOLIDWORKS\Toolbox"
            };
            ...
            foreach (string raw in roots)
            {
                ...
                string standardRoot = string.IsNullOrWhiteSpace(standard)
                    ? null
                    : Path.Combine(root, standard);
                if (!string.IsNullOrEmpty(standardRoot) &&
                    Directory.Exists(standardRoot) &&
                    seen.Add(standardRoot))
                    yield return standardRoot;
                if (seen.Add(root)) yield return root;
            }
        }

        private static string ResolveToolboxPart(
            string explicitRoot,
            string standard,
            string partNumber)
        {
            string query = (partNumber ?? "").Trim();
            ...
            {
                string mapped;
                if (ToolboxNameHints.TryGetValue(match.Value, out mapped))
                {
                    hint = mapped;
                    break;
                }
            ...
            string best = null;
            int bestScore = -1;
            foreach (string root in ToolboxRoots(explicitRoot, standard))
            {
                IEnumerable<string> files;
                try
                {
                    files = Directory.EnumerateFiles(
                        root, "*.sldprt", SearchOption.AllDirectories);
                }
                catch
                {
                    continue;
                ...
                {
                    // Some Toolbox subfolders can be inaccessible; retain the best match so far.
                }
            }
            return bestScore > 0 ? best : null;
        }

        public ExecutionResponse InsertToolboxComponent(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            {
                var p = request.Params as JObject;
                string partNumber = p?.Value<string>("part_number");
                if (string.IsNullOrWhiteSpace(partNumber))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "MISSING_PARAMETER", "part_number is required.");

                var modelDoc = _solidWorks.IActiveDoc2 as IModelDoc2;
                if (!(modelDoc is IAssemblyDoc))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "WRONG_DOCUMENT_TYPE",
                        "insert_toolbox_component requires an active ASSEMBLY document.");

                string standard = p?.Value<string>("standard") ?? "gb";
                string selectedPath = ResolveToolboxPart(
                    p?.Value<string>("toolbox_root"),
                    standard,
                    partNumber);
                if (string.IsNullOrEmpty(selectedPath))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "TOOLBOX_PART_NOT_FOUND",
                        $"No Toolbox part matched '{partNumber}' under the configured Toolbox roots.");

                var delegatedParams = new JObject
                {
                    ["file_path"] = selectedPath,
                    ["config"] = p?.Value<string>("configuration") ?? "",
                    ["x"] = p?.Value<double?>("x") ?? 0.0,
                    ["y"] = p?.Value<double?>("y") ?? 0.0,
                    ["z"] = p?.Value<double?>("z") ?? 0.0
                };
                if (p?["fixed"] != null)
                    delegatedParams["fixed"] = p.Value<bool>("fixed");

                var delegated = InsertComponent(new ToolRequest
                {
                    OperationId = request.OperationId,
                ...
                if (delegated.Status == "COMPLETED" && delegated.CadState != null)
                {
                    delegated.CadState.Features.Insert(
                        0, "toolbox_path=" + selectedPath);
                    delegated.ResultGeometry = new JObject
                    {
                        ["toolbox_path"] = selectedPath,
                        ["standard"] = standard,
                        ["part_number"] = partNumber,
                        ["configuration"] = p?.Value<string>("configuration") ?? ""
                    };
                }
                return delegated;
            }
        ...
        public ExecutionResponse CreateExplodedView(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            try
            {
                ...
                if (assembly == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "WRONG_DOCUMENT_TYPE",
                        "create_exploded_view requires an active ASSEMBLY document.");
                if (assembly.GetComponentCount(false) == 0)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "EMPTY_ASSEMBLY",
                        "The active assembly has no components to explode.");

                if (!assembly.AutoExplode())
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "AUTO_EXPLODE_FAILED",
                        "SolidWorks could not calculate an automatic exploded view.");

                bool created = assembly.CreateExplodedView();
                string[] names = assembly.GetExplodedViewNames2("") as string[];
                if (!created && (names == null || names.Length == 0))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "EXPLODED_VIEW_FAILED",
                        "SolidWorks did not create an exploded-view configuration.");

                var p = request.Params as JObject;
                string requestedName = p?.Value<string>("view_name");
                if (!string.IsNullOrWhiteSpace(requestedName))
                    assembly.ShowExploded2(true, requestedName);
                modelDoc.EditRebuild3();

                names = assembly.GetExplodedViewNames2("") as string[];
                return BuildCompleted(
                    request,
                    modelDoc,
                    true,
                    new List<string>(names ?? new string[0]),
                    resultGeometry: new JObject
                    {
                        ["created"] = true,
                        ["requested_view"] = requestedName ?? "",
                        ["view_count"] = names == null ? 0 : names.Length
                    });
            }
            catch (COMException ex)
            {
