...
namespace SolidworksExecution.Services
{
    ...
    public partial class SolidWorksService
    {
        ...
        public ExecutionResponse CreateDetailView(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            {
                var p = request.Params as JObject;
                string sourceViewName = p?.Value<string>("source_view");
                if (string.IsNullOrWhiteSpace(sourceViewName))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "MISSING_PARAMETER", "source_view is required.");

                double centerXRatio = p?.Value<double?>("center_x_ratio") ?? 0.5;
                double centerYRatio = p?.Value<double?>("center_y_ratio") ?? 0.5;
                double radiusRatio = p?.Value<double?>("radius_ratio") ?? 0.18;
                double detailScale = p?.Value<double?>("scale") ?? 2.0;
                string label = p?.Value<string>("label") ?? "B";
                if (centerXRatio < 0 || centerXRatio > 1 ||
                    centerYRatio < 0 || centerYRatio > 1 ||
                    radiusRatio <= 0 || radiusRatio >= 0.5 ||
                    detailScale <= 0)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "INVALID_PARAMETER",
                        "center ratios must be between 0 and 1, radius_ratio must be between 0 and 0.5, and scale must be positive.");

                var modelDoc = _solidWorks.IActiveDoc2 as IModelDoc2;
                var drawingDoc = modelDoc as IDrawingDoc;
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        modelDoc == null ? "NO_ACTIVE_DOCUMENT" : "NOT_A_DRAWING",
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "create_detail_view requires an active DRAWING document.");

                var source = FindModelView(drawingDoc, sourceViewName);
                if (source == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_NOT_FOUND", "No drawing view named '" + sourceViewName + "' was found.");

                double[] outline = DrawingViewOutline(source);
                double width = outline[2] - outline[0];
                double height = outline[3] - outline[1];
                double centerX = outline[0] + width * centerXRatio;
                double centerY = outline[1] + height * centerYRatio;
                double radius = Math.Max(0.002, Math.Min(width, height) * radiusRatio);

                modelDoc.ClearSelection2(true);
                if (!drawingDoc.ActivateView(source.Name))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_ACTIVATION_FAILED",
                        "Could not activate detail-view source '" + source.Name + "'.");

                double[] localCenter = TransformSheetToViewSketch(source, centerX, centerY);
                double[] localRadius = TransformSheetToViewSketch(
                    source, centerX + radius, centerY);
                if (localCenter == null || localRadius == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_TRANSFORM_FAILED",
                        "Could not transform the detail boundary into the source view sketch.");

                var circle = modelDoc.SketchManager.CreateCircle(
                    localCenter[0], localCenter[1], 0,
                    localRadius[0], localRadius[1], 0);
                if (circle == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "DETAIL_BOUNDARY_FAILED",
                        "SolidWorks could not create the detail boundary circle.");

                var destination = FindDrawingViewPlacement(
                    drawingDoc, source,
                    Math.Max(width * detailScale, 0.05),
                    Math.Max(height * detailScale, 0.05));
                var detailView = drawingDoc.CreateDetailViewAt4(
                    destination.Item1,
                    destination.Item2,
                    0,
                ...
                if (detailView == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "DETAIL_VIEW_FAILED",
                        "SolidWorks could not create a detail view from the selected boundary.");

                try { detailView.SetName2("DSH_Detail_" + label); } catch { }
                modelDoc.ClearSelection2(true);
                ...
                return BuildCompleted(
                    ...
                    modelDoc,
                    true,
                    new List<string> { detailView.Name },
                    resultGeometry: new JObject
                    {
                        ["detail_view"] = detailView.Name,
        ...
        public ExecutionResponse ImportModelAnnotations(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            try
            {
                ...
                bool eliminateDuplicates =
                    p?.Value<bool?>("eliminate_duplicates") ?? true;
                ...
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "import_model_annotations requires an active DRAWING document.");
                ...
                object inserted = drawingDoc.InsertModelAnnotations3(
                    ...
                    false,
                    true);
                int insertedCount = CountComItems(inserted);
                modelDoc.GraphicsRedraw2();
                ...
                return BuildCompleted(
                    ...
                    dimensions: new List<string>
                    {
                        "model_annotations=" + insertedCount
                    },
                    resultGeometry: new JObject
                    {
                        ["inserted_count"] = insertedCount,
        ...
        public ExecutionResponse CreateBom(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            try
            {
                ...
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "create_bom requires an active DRAWING document.");

                IView assemblyView = null;
                foreach (var candidate in EnumerateModelViews(drawingDoc))
                {
                    try
                    {
                        if (candidate.ReferencedDocument != null &&
                            candidate.ReferencedDocument.GetType() ==
                            (int)swDocumentTypes_e.swDocASSEMBLY)
                        {
                            assemblyView = candidate;
                            break;
                        }
                    }
                    catch { }
                ...
                if (assemblyView == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "ASSEMBLY_VIEW_NOT_FOUND",
                        "The drawing has no resolved assembly view from which to create a BOM.");
                ...
                if (!string.IsNullOrWhiteSpace(templatePath) &&
                    ...
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        "The requested BOM template does not exist: " + templatePath);
                if (string.IsNullOrWhiteSpace(templatePath))
                    templatePath = ResolveBomTemplatePath();
                if (string.IsNullOrWhiteSpace(templatePath))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "BOM_TEMPLATE_NOT_FOUND",
                        "No SolidWorks BOM template was found. Provide template_path.");

                string typeName = p?.Value<string>("bom_type") ?? "top_level";
                int bomType;
                switch (typeName.Trim().ToLowerInvariant())
                {
                    ...
                    default:
                        return BuildFailed(request.OperationId,
                            ...
                            "INVALID_BOM_TYPE",
                            "bom_type must be top_level, parts_only, or indented.");
                }

                var table = assemblyView.InsertBomTable4(
                    false,
                    0.015,
                    0.015,
                    (int)swBOMConfigurationAnchorType_e.swBOMConfigurationAnchor_BottomLeft,
                    bomType,
                    assemblyView.ReferencedConfiguration ?? "",
                    templatePath,
                    false,
                    0,
                    false);
                if (table == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "BOM_CREATION_FAILED",
                        "SolidWorks could not create a BOM from the assembly view.");
                ...
                return BuildCompleted(
                    ...
                    modelDoc,
                    true,
                    new List<string> { "BOM@" + assemblyView.Name },
                    resultGeometry: new JObject
                    {
                        ["source_view"] = assemblyView.Name,
                        ["configuration"] =
                            assemblyView.ReferencedConfiguration ?? "",
                        ["bom_type"] = typeName,
        ...
        public ExecutionResponse AddBalloons(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            try
            {
                ...
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "add_balloons requires an active DRAWING document.");

                string viewName = p?.Value<string>("view_name");
                IView target = string.IsNullOrWhiteSpace(viewName)
                    ? null
                    : FindModelView(drawingDoc, viewName);
                if (target == null && !string.IsNullOrWhiteSpace(viewName))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_NOT_FOUND",
                        "No drawing view named '" + viewName + "' was found.");
                if (target == null)
                {
                    foreach (var candidate in EnumerateModelViews(drawingDoc))
                    {
                ...
                if (target == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_NOT_FOUND",
                        "The drawing has no model view to receive balloons.");

                modelDoc.ClearSelection2(true);
                if (!drawingDoc.ActivateView(target.Name))
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_ACTIVATION_FAILED",
                        "Could not activate balloon target '" + target.Name + "'.");

                string style = p?.Value<string>("style") ?? "circular";
                var options = new AutoBalloonOptionsClass
                {
                    Layout = (int)swBalloonLayoutType_e.swDetailingBalloonLayout_Top,
                    Style = BalloonStyleValue(style),
                    Size = (int)swBalloonFit_e.swBF_2Chars,
                    IgnoreMultiple = false,
                    LeaderAttachmentToFaces = false,
                    ReverseDirection = false,
                    ItemOrder =
                        (int)swBalloonItemNumbersOrder_e.swBalloonItemNumbers_OrderSequentially
                };
                int balloonCount = CountComItems(drawingDoc.AutoBalloon5(options));
                if (balloonCount == 0)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "BALLOON_CREATION_FAILED",
                        "SolidWorks created no balloons. Ensure the view references an assembly and has a BOM.");
                ...
                return BuildCompleted(
                    ...
                    new List<string>
                    {
                        "balloons=" + balloonCount + "@" + target.Name
                    },
                    resultGeometry: new JObject
                    {
        ...
        public ExecutionResponse CreateHoleTable(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            try
            {
                ...
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "create_hole_table requires an active DRAWING document.");

                string viewName = p?.Value<string>("view_name");
                IView target = string.IsNullOrWhiteSpace(viewName)
                    ? FirstModelView(drawingDoc)
                    : FindModelView(drawingDoc, viewName);
                if (target == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "VIEW_NOT_FOUND",
                        string.IsNullOrWhiteSpace(viewName)
                            ? "The drawing has no model view from which to create a hole table."
                            : "No drawing view named '" + viewName + "' was found.");

                string tagStyle = p?.Value<string>("tag_style") ?? "numeric";
                int tagType;
                switch (tagStyle.Trim().ToLowerInvariant())
                {
                    case "numeric":
                        tagType =
                            (int)swHoleTableTagStyle_e.swHoleTable_NumericTags;
                        break;
                    case "alphanumeric":
                        tagType =
                            (int)swHoleTableTagStyle_e.swHoleTable_AlphaNumericTags;
                        break;
                    default:
                        return BuildFailed(request.OperationId,
                            _guard.GetCurrentStateVersion(),
                            "INVALID_TAG_STYLE",
                            "tag_style must be numeric or alphanumeric.");
                }

                double[] outline = DrawingViewOutline(target);
                modelDoc.ClearSelection2(true);
                object table = target.InsertHoleTable3(
                    false,
                    outline[2] + 0.02,
                    outline[1],
                    0,
                    "1",
                    "",
                    (int)swHoleTableTagOrder_e.swHoleTableTagOrder_XY,
                    tagType,
                    null);
                if (table == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        "HOLE_TABLE_CREATION_FAILED",
                        "SolidWorks could not create a hole table. The view may contain no recognized holes.");
                ...
                return BuildCompleted(
                    ...
                    modelDoc,
                    true,
                    new List<string> { "hole_table@" + target.Name },
                    resultGeometry: new JObject
                    {
                        ["source_view"] = target.Name,
                        ["tag_style"] = tagStyle
                    });
            }
            catch (COMException ex)
        ...
        public ExecutionResponse CheckDrawingStandards(ToolRequest request)
        {
            if (_guard.IsDuplicate(request.OperationId))
                return _guard.GetDuplicate(request.OperationId);
            if (!_guard.IsStateVersionValid(request.StateVersion))
            ...
            {
                var p = request.Params as JObject;
                int expectedViewCount =
                    p?.Value<int?>("expected_view_count") ?? 0;
                bool requireDimensions =
                    p?.Value<bool?>("require_dimensions") ?? true;
                bool requireResolvedReferences =
                    p?.Value<bool?>("require_resolved_references") ?? true;
                int maxIndependentScales =
                    p?.Value<int?>("max_independent_scales") ?? 0;
                if (expectedViewCount < 0 || maxIndependentScales < 0)
                    return BuildFailed(request.OperationId,
                        _guard.GetCurrentStateVersion(),
                        "INVALID_PARAMETER",
                        "expected_view_count and max_independent_scales must be non-negative.");
                ...
                if (drawingDoc == null)
                    return BuildFailed(request.OperationId, _guard.GetCurrentStateVersion(),
                        ...
                        modelDoc == null
                            ? "No active document found in SolidWorks."
                            : "check_drawing_standards requires an active DRAWING document.");

                bool rebuilt = modelDoc.ForceRebuild3(false);
                var views = new List<IView>(EnumerateModelViews(drawingDoc));
                int dimensions = 0;
                int annotations = 0;
                int danglingDimensions = 0;
                int independentScales = 0;
                var missingReferences = new JArray();

                foreach (var view in views)
                {
                    bool referenceResolved = false;
                    try { referenceResolved = view.ReferencedDocument != null; }
                    catch { }
                    if (!referenceResolved)
                    {
                        try
                        {
                            string path = view.GetReferencedModelName();
                            referenceResolved =
                    ...
                    if (!referenceResolved) missingReferences.Add(view.Name);

                    try
                    {
                        dimensions += Math.Max(
                            0, view.GetDisplayDimensionCount());
                    }
                    catch { }
                    try
                    {
                        annotations += Math.Max(0, view.GetAnnotationCount());
                    }
                    catch { }
                    try
                    {
                        if (view.UseSheetScale == 0 &&
                            !IsSectionOrDetailView(view))
                            independentScales++;
                    }
                    catch { }
                ...
                if (!rebuilt) violations.Add("drawing_rebuild_failed");
                if (views.Count == 0) violations.Add("no_model_views");
                if (expectedViewCount > 0 &&
                    views.Count < expectedViewCount)
                    violations.Add("view_count_below_expected");
                if (requireDimensions && dimensions == 0)
                    violations.Add("no_display_dimensions");
                if (requireResolvedReferences &&
                    missingReferences.Count > 0)
                    violations.Add("unresolved_model_references");
                if (independentScales > maxIndependentScales)
                    violations.Add("too_many_independent_scales");
                if (danglingDimensions > 0)
                    violations.Add("dangling_dimensions");

                var report = new JObject
                {
                    ["passed"] = violations.Count == 0,
                    ["rebuild_passed"] = rebuilt,
                    ["view_count"] = views.Count,
                    ["expected_view_count"] = expectedViewCount,
                    ["dimension_count"] = dimensions,
                    ["annotation_count"] = annotations,
                    ["dangling_dimension_count"] = danglingDimensions,
                    ["independent_scale_count"] = independentScales,
                    ["max_independent_scales"] = maxIndependentScales,
                ...
                if (violations.Count > 0)
                {
                    var failed = BuildFailed(
                        request.OperationId,
                        _guard.GetCurrentStateVersion(),
                        "DRAWING_STANDARDS_FAILED",
                        "Drawing standards check failed: " +
                        string.Join(", ", violations));
                    failed.ResultGeometry = report;
        ...
        private static IEnumerable<IView> EnumerateModelViews(
            IDrawingDoc drawingDoc)
        {
            object current = drawingDoc.GetFirstView();
            var sheetView = current as IView;
            current = sheetView != null ? sheetView.GetNextView() : null;
            int guard = 0;
        ...
        private static IView FirstModelView(IDrawingDoc drawingDoc)
        {
            foreach (var view in EnumerateModelViews(drawingDoc))
                return view;
            return null;
        }

        private static IView FindModelView(
            IDrawingDoc drawingDoc,
            string name)
        {
        ...
        private static double[] DrawingViewOutline(IView view)
        {
            ...
            if (outline == null || outline.Length < 4 ||
                ...
                throw new InvalidOperationException(
                    "Could not read drawing view outline.");
            return outline;
        }

        private static bool IsSectionOrDetailView(IView view)
        {
            if (view == null) return false;
            try { if (view.GetSection() != null) return true; } catch { }
            try { if (view.GetDetail() != null) return true; } catch { }
            return false;
        }

        private static Tuple<double, double> FindDrawingViewPlacement(
            IDrawingDoc drawingDoc,
            IView source,
            double desiredWidth,
            double desiredHeight)
        {
            double sheetWidth = 0.42;
            double sheetHeight = 0.297;
            try
            {
            ...
            double[] sourceOutline = DrawingViewOutline(source);
            desiredWidth = Math.Min(
                Math.Max(desiredWidth, 0.03), sheetWidth * 0.42);
            desiredHeight = Math.Min(
                Math.Max(desiredHeight, 0.03), sheetHeight * 0.42);
            double centerX =
                (sourceOutline[0] + sourceOutline[2]) / 2;
            double centerY =
                (sourceOutline[1] + sourceOutline[3]) / 2;
            double gap = 0.018;
            var candidates = new[]
            {
                Tuple.Create(
                    sourceOutline[2] + gap + desiredWidth / 2,
                    centerY),
                Tuple.Create(
                    sourceOutline[0] - gap - desiredWidth / 2,
                    centerY),
                Tuple.Create(
                    centerX,
                    sourceOutline[3] + gap + desiredHeight / 2),
                Tuple.Create(
                    centerX,
                    sourceOutline[1] - gap - desiredHeight / 2),
                Tuple.Create(sheetWidth * 0.72, sheetHeight * 0.68)
            };

            var existing = new List<double[]>();
            foreach (var view in EnumerateModelViews(drawingDoc))
            {
                try { existing.Add(DrawingViewOutline(view)); }
                catch { }
            }
            foreach (var candidate in candidates)
            {
                double x = Math.Max(
                    0.015 + desiredWidth / 2,
                    Math.Min(
                        sheetWidth - 0.015 - desiredWidth / 2,
                        candidate.Item1));
                double y = Math.Max(
                    0.02 + desiredHeight / 2,
                    Math.Min(
                        sheetHeight - 0.015 - desiredHeight / 2,
                        candidate.Item2));
                double left = x - desiredWidth / 2 - 0.008;
                double right = x + desiredWidth / 2 + 0.008;
                double bottom = y - desiredHeight / 2 - 0.008;
                double top = y + desiredHeight / 2 + 0.008;
                bool overlaps = false;
                foreach (var outline in existing)
                {
                    if (!(right < outline[0] ||
                          left > outline[2] ||
                          top < outline[1] ||
                ...
                if (!overlaps) return Tuple.Create(x, y);
            }
            return Tuple.Create(
                sheetWidth * 0.72,
                sheetHeight * 0.68);
        ...
        private static int CountComItems(object value)
        {
        ...
        private static int BalloonStyleValue(string style)
        {
            switch ((style ?? "").Trim().ToLowerInvariant())
            {
                case "square":
                    return (int)swBalloonStyle_e.swBS_Square;
                case "hexagon":
                    return (int)swBalloonStyle_e.swBS_Hexagon;
                case "triangle":
                    return (int)swBalloonStyle_e.swBS_Triangle;
                case "box":
                    return (int)swBalloonStyle_e.swBS_Box;
                case "diamond":
                    return (int)swBalloonStyle_e.swBS_Diamond;
                default:
                    return (int)swBalloonStyle_e.swBS_Circular;
            }
        }

        private static string ResolveBomTemplatePath()
        {
            string installDir = System.Environment.GetEnvironmentVariable(
                "SOLIDWORKS_INSTALL_DIR");
            var candidates = new List<string>
            {
                string.IsNullOrWhiteSpace(installDir) ? null : System.IO.Path.Combine(
                    installDir,
                    "lang",
                    "chinese-simplified",
